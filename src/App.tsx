...
import { streamChatResponse, streamDeepResearch } from './services/api';
...
const App = () => {
  ...
  const handleScroll = () => {
    ...
    // If the user scrolls up more than 150px from the bottom, they have manually scrolled
  ...
  useEffect(() => {
    // When a new message is added by the user, we want to force scroll to bottom.
    // We can detect this if the last message is from the user.
    const lastMessage = messages[messages.length - 1];
    if (lastMessage && lastMessage.role === 'user') {
      setUserScrolledUp(false); // Reset scroll state on new user message
      setTimeout(() => {
    ...
    if (shouldSearch) {
        ...
        await streamDeepResearch(
            ...
            (progressEvent) => {
                setMessages(prev => prev.map(m => {
                    if (m.id === aiMessageId) {
                        const currentEvents = m.progressEvents || [];
                        // Update existing event for same step, or add new one
                        const existingIndex = currentEvents.findIndex(e => e.step === progressEvent.step);
                        let newEvents = [...currentEvents];
                        if (existingIndex >= 0) {
                            newEvents[existingIndex] = progressEvent;
                        } else {
                            newEvents.push(progressEvent);
                        }
                        return { ...m, progressEvents: newEvents };
                    }
                    return m;
                }));
            },
            (researchEvent) => {
                setMessages(prev => prev.map(m => {
                    if (m.id === aiMessageId) {
                        const currentSteps = m.researchSteps || [];
                        if (researchEvent.type === 'research_step') {
                            return { ...m, researchSteps: [...currentSteps, researchEvent.data] };
                        } else if (researchEvent.type === 'research_step_update') {
                            const index = currentSteps.findIndex(s => s.id === researchEvent.data.id);
                            if (index >= 0) {
                                const newSteps = [...currentSteps];
                                newSteps[index] = { ...newSteps[index], ...researchEvent.data };
            ...
            () => {
                ...
                if (matches) {
                    questions = matches.map(m => m.replace(/<<|>>/g, '').trim()).slice(0, 3);
                }
                
                setMessages(prev => prev.map(msg => {
                    if (msg.id === aiMessageId) {
                        const updatedEvents = (msg.progressEvents || []).map(e => ({ ...e, status: 'done' as const }));
                        return { 
                            ...msg, 
                            relatedQuestions: questions.length > 0 ? questions : msg.relatedQuestions,
            ...
            (error) => {
                ...
                setMessages(prev => prev.map(m => m.id === aiMessageId ? { ...m, isSearching: false, content: m.content + '\n\n[System: Deep Research failed]' } : m));
                setIsLoading(false);
                abortControllerRef.current = null;
            },
  ...
  return (
    <div className="flex h-screen bg-background text-foreground font-sans overflow-hidden transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col relative h-full w-full max-w-full">
        {/* Top Navigation */}
        <div className="h-16 flex items-center justify-between px-4 md:px-6 absolute top-0 w-full z-[60]">
        ...
        {/* Chat Area */}
        <div 
          className="flex-1 overflow-y-auto w-full pb-52" 
          id="chat-container" 
                ...
                <div key={msg.id} className="group mb-8 animate-fade-in flex flex-col gap-2">
                   ...
                   <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0 overflow-hidden">
                            ...
                            {/* PROGRESS WIDGET */}
                            {msg.role === 'assistant' && msg.progressEvents && msg.progressEvents.length > 0 && !msg.isSearching && (
                                <ProgressWidget events={msg.progressEvents} />
                            )}

                            {/* RESEARCH TIMELINE */}
                            {msg.role === 'assistant' && msg.researchSteps && msg.researchSteps.length > 0 ? (
                                <ResearchTimeline 
                                  steps={msg.researchSteps}
                                  sources={msg.searchResults || []}
                                  isSearching={msg.isSearching || false}
                                />
                            ) : null}

                            {/* SEARCH WIDGET (Only show if no research timeline) */}
                            {msg.role === 'assistant' && msg.searchResults && msg.searchResults.length > 0 && !(msg.researchSteps && msg.researchSteps.length > 0) && (
                                <SearchWidget 
        ...
        {hasMessages && (
          <div className="absolute bottom-0 left-0 right-0 pointer-events-none z-10 flex flex-col">
            <div className="h-4 w-full bg-gradient-to-t from-background to-transparent" />
            ...
            <div className="max-w-3xl mx-auto relative">
                ...
                {!hasMessages && (
                  <div className="flex flex-col items-center justify-center mb-6 animate-fade-in">
                    <div className="mb-3 w-full flex justify-center"><Logo className="h-10 sm:h-11 md:h-12 text-foreground" /></div>
                  </div>
                )}
                ...
                <div ref={inputContainerRef} onClick={handleContainerClick} className="relative bg-secondary rounded-[32px] p-4 transition-colors duration-200 cursor-text group shadow-lg border border-border flex flex-col gap-3">
                    ...
                    <textarea
                        ...
                        className="w-full bg-transparent text-foreground text-lg placeholder-muted-foreground px-2 outline-none resize-none max-h-[120px] md:max-h-[200px] overflow-y-auto min-h-[24px] leading-relaxed custom-scrollbar-hidden"
                        rows={1}
                    />
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="relative" ref={modelMenuRef}>
                                <button onClick={(e) => { e.stopPropagation(); setIsModelMenuOpen(!isModelMenuOpen); }} className="h-10 px-4 rounded-full bg-background hover:bg-muted text-muted-foreground hover:text-foreground text-sm font-medium transition-colors flex items-center gap-2">
                                 ...
                                 {isModelMenuOpen && (
                                    <div className={`absolute left-0 w-72 max-h-[220px] md:max-h-80 overflow-y-auto bg-popover border border-border rounded-xl shadow-xl z-[60] flex flex-col gap-0.5 animate-fade-in-up bottom-full mb-2 custom-scrollbar`}>
                                        <div className="px-4 py-3 text-xs font-semibold text-muted-foreground sticky top-0 bg-popover z-10 border-b border-border/50">Select Model</div>
